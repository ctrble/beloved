...
static var instance : UseBottle; //so this script can be called from other scripts
...
function Update () {
	...
	if(Input.GetButton ("LeftClick")){ //input left click, should find a way to avoid affecting other held items, if it even acts weird
		if(pickupDropScript.bottle.IsChildOf(pickupDropScript.leftHand)){ //is it in the left hand?
	...
	if(Input.GetButton ("RightClick")){ //input left click, should find a way to avoid affecting other held items, if it even acts weird
		if(pickupDropScript.bottle.IsChildOf(pickupDropScript.rightHand)){ //is it in the right hand?
		renderer.material.color = Color.blue; //DEBUG : does this whole script do anything?
		}
	}
...
function OnTriggerEnter (myTrigger : Collider) { //checking if in range of item
	Debug.Log ("You are closest to the bottle");
	pickupDropScript.closestItem = pickupDropScript.bottle; //talk to PickupDrop and set a variable to be this item

}

function IsHeld () {
	isHeld = true; //holding this item
	Debug.Log ("You're holding a bottle");

}

function IsDropped () {
	isHeld = false; //not holding this item
	Debug.Log ("You dropped a bottle");
